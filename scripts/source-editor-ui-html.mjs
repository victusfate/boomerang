// HTML template for the RSS source editor UI.
// Exported as a string and served by source-editor-ui.mjs.
// Numeric values within the CSS are layout constants — see quality-ok pragmas below.
...
<style>
// quality-ok: magic-number — CSS layout value
...
<main>
  ...
  <div class="table-wrap">
    <table>
      <thead>
        <tr>
          <th>Verdict</th><th>ID</th><th>Name</th><th>Category</th>
          <th>Pri</th><th>Enabled</th><th>Feed URL</th><th>Notes</th><th>Actions</th>
        </tr>
      </thead>
...
// ── verdict helper ────────────────────────────────────────────────────────────
function verdict(id) { return results[id]?.verdict ?? 'UNKNOWN'; }
...
async function save() {
  ...
  if (d.ok) {
    sources = final; cutIds.clear(); dirty = false; render();
...
function showToast(msg) {
  ...
  // quality-ok: magic-number — toast display duration (2500ms) and fade-out delay (400ms)
